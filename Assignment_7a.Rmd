...
```
It can be inferred from the graph that the name Aaron was relatively uncommon before 1960. However, after the 60s it was sparsely somewhat popular in some of the years.
...
```{r}
bYear <- babynames %>% filter(year == 1996, sex == "M") %>% select(name, prop) %>% arrange(sort(prop, decreasing = F))
gYear <- babynames %>% filter(year == 1996, sex == "F") %>% select(name, prop) %>% arrange(sort(prop))
slice(bYear, 1:10)
slice(gYear, 1:10)
```
...
babynames %>% mutate(firstLetter = substr(name, 1, 1)) %>% filter(firstLetter == "R" | firstLetter == "J", sex == "M") %>%  group_by(year, firstLetter) %>% summarise(frequency = sum(n)) -> popFirstLetter
